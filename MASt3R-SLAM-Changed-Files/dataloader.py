...
#Steve added  
class NodeJSWebSocketCamera(MonocularDataset):
    def __init__(self, ws_url):
        super().__init__()
        self.use_calibration = False
        self.ws_url = ws_url
        self.dataset_path = pathlib.Path("esp32_stream")
        self.latest_frame = None
        self.count = 0
        self.lock = threading.Lock()
        self.img_size = 512 
        
        # FIX 1: Provide dummy data so the parent class logic doesn't crash 
        # before your overrides take over.
    ...
    def read_img(self, idx):
        ...
        with self.lock:
            img = self.latest_frame.copy()  # was 'mg'
    ...
    def get_img_shape(self):
        ...
        # Apply the same crop logic as read_img
        h, w = frame.shape[:2]
        size = min(h, w)  # 480 for a 640x480 frame
        top  = (h - size) // 2
        left = (w - size) // 2
        cropped = frame[top:top+size, left:left+size]
        
        # Now ask mast3r what it will actually produce from this square input
        resized = resize_img(cropped, self.img_size)
        processed_shape = resized["img"][0].shape[1:]  # H, W after resize
        raw_shape = (size, size)
        
        return processed_shape, raw_shape
...
def load_dataset(dataset_path):
    ...
    if "ws" in dataset_path:
        return NodeJSWebSocketCamera(dataset_path)
